...
cell lines, and what the structural data suggests.`,
  ...
  const copyExperimentId = () => {
    const citation = `Dialectic Experiment Report · ${new Date().toLocaleDateString()} · ${privateNode?.subject_name} vs ${litNode?.subject_name} · DiffDock score: ${results?.direction_a?.diffdock_score?.toFixed(3) ?? "N/A"} · Friction: ${privateNode?.friction_score?.toFixed(2)} · Generated by Dialectic (Anthropic Claude + Tamarind Bio DiffDock)`;
          ...
          {results ? (
            <>
              ...
              <div style={{ fontSize: 9, color: "#3a4055", marginBottom: 8 }}>
                Generated by Claude · Grounded in DiffDock structural data
              </div>
              ...
              <div style={s.exportRow}>
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    window.dispatchEvent(new CustomEvent("dialectic:create-bag", {
                      detail: {
                        name: bagName,
                        nodeIds: [privateNode?.node_id, litNode?.node_id].filter(Boolean),
                      },
                    }));
                    setSavedToBag(true);
                    setTimeout(() => setSavedToBag(false), 2000);
                  }}
                >
                  {savedToBag ? "✓ Saved!" : "⊟ Save to Bag"}
                </button>
